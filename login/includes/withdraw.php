...
if ($code != $verification) {
    ...
    if(isset($_POST['submit'])){ 
        ...
        if ($input == ""){
            ...
            $bdb = new DbConn;
            $bstmt = $bdb->conn->prepare("UPDATE balance SET pendingOut = :pending WHERE username = :username");
            $bstmt->bindParam(':pending', $clearPending);
            $bstmt->bindParam(':username', $username);
            $bstmt->execute();

            $m = new MailSender;
            $m->sendReasonMail($email['email'], $username, $amount, $input, 'WithdrawReject');

            logAction("WITHDRAWAL REJECT", $username, "Withdrawal request for ".$amount." rejected for reason: ".$input);
        }
...
<?php 
    ...
    $m = new MailSender;
    $m->sendAcceptMail($email['email'], $username, $amount, 'WithdrawalAccept');

    logAction("WITHDRAW", $username, "Withdrawal request for ".$amount." accepted.");
